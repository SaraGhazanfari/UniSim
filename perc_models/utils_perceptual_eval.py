...
from perc_models.utils_perceptual_models import PRETRAINED_MODELS
...
def get_sim(outputs, logger=None, verbose=True):
    ...
    for emb_ref, emb_left, emb_right, lab, id in outputs:
        ...
        if verbose:
            print(f'[cum] acc={acc / n_ex:.1%} ({acc:.0f}/{n_ex})')
...
@torch.no_grad()
def get_acc(model, loader, device, cust_im_ref=None, n_ex=-1, mode='',
    ...
    if mode == 'lpips':
        ...
        _, clean_acc, _ = get_sim(output_clean, logger, verbose=verbose)
        return clean_acc
        ...
        for i, (im0, im1, im2, lab, _) in enumerate(loader):
            ...
            if verbose:
                print(f'[cum] acc={acc / seen_imgs:.1%} ({acc:.0f}/{seen_imgs})')
            if n_ex != -1 and seen_imgs >= n_ex:
                break
...
def resolve_args(args):
    ...
    model_info = PRETRAINED_MODELS.get(args.shortname, None)
    ...
    if args.ckptpath is not None:
        ...
        if 'R-LPIPS' in args.ckptpath:
            args.source = 'r-lpips'
    if args.dataset in ['things', 'cifar100coarse-triplets', 'imagenet-triplets',
                        'r-imagenet-triplets', 'bal-r-imagenet-triplets', 'h-imagenet-triplets']:
