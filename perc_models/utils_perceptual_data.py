...
def load_dataset(args, **kwargs):
    ...
    if args.dataset == 'nights':
        ...
        ds = THINGSDataset(
            _data_dir,
            os.path.join(args.data_dir, DEFAULT_DIRS['things-imgpaths']),
            os.path.join(args.data_dir, DEFAULT_DIRS['things-triplets']),
...
if __name__ == '__main__':
    ...
    args.batch_size = 10
    args.dataset = ['nights', 'ir-1k', 'things'][-1]
    args.device = 'cuda:0'
    args.modelname = None
    args.ckptpath = None
    args.norm = None
    args.attack_name = None
    args.logdir = './'
    args.savedir = './'
    args.split = ['test_imagenet', 'none'][-1]
    args.n_ex = -1
    args.data_dir = '/tmlscratch/fcroce/datasets/'

    logger = Logger(None)

    ds, loader = load_dataset(args, preprocess=lambda x: x, logger=logger)
    print('Data loaded.')
